...
export default function App() {
  ...
  useEffect(() => {
    if (window.gsap && window.ScrollTrigger) {
      ...
      if (boxes.length > 0) {
        window.gsap.set(boxes, { transformPerspective: 1200, transformStyle: "preserve-3d" });
      }

      // 1. HERO INITIAL SEQUENTIAL ENTRANCE TIMELINE
      const seqTl = window.gsap.timeline({ defaults: { ease: "power3.out" } });

      seqTl
        .fromTo(line1Ref.current,
          { opacity: 0, y: 20, filter: "blur(4px)" },
          { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.45, delay: 0.05 }
        )
